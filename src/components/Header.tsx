        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-3.5">
          ...
          <Link href="/" className="flex items-center gap-3">
            <span className="relative h-11 w-11 overflow-hidden rounded-full ring-1 ring-ink/10">
              <Image src="/images/logo-header-round.png" alt="Set of Decore" fill sizes="44px" priority className="object-cover" />
            </span>
            <span className="flex flex-col leading-none">
              <span className="font-display text-xl tracking-tight">Set of Decore</span>
              <span className="mt-0.5 text-[10px] uppercase tracking-[0.18em] text-taupe">
                Home Textiles &amp; More
              </span>
            </span>
          </Link>
          ...
          <nav className="hidden gap-7 text-sm md:flex">
            {NAV_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="relative text-ink/75 transition hover:text-ink after:absolute after:-bottom-1 after:left-0 after:h-px after:w-0 after:bg-ink after:transition-all hover:after:w-full"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-5">
            <Link href="/search" aria-label="Search" className="text-ink/80 transition hover:text-ink">
              <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                <circle cx="8" cy="8" r="6" stroke="currentColor" strokeWidth="1.5" />
                <path d="M16 16l-3.5-3.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
              </svg>
            </Link>
            <Link href="/account" aria-label="Account" className="hidden text-ink/80 transition hover:text-ink md:block">
              <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
            ...
            <Link href="/cart" aria-label="Cart" className="relative text-ink/80 transition hover:text-ink">
